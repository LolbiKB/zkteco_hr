...
/**
 * Base types for generic data table components
 * These interfaces can be extended for specific entity tables
 */
...
export interface TableConfig {
  ...
  createButtonIcon?: React.ComponentType<{ className?: string }>
}
...
interface GenericDataTableProps<TData, TFilters extends BaseFilters> {
  ...
  hideToolbar?: boolean
  toolbarActions?: React.ReactNode  // Right-side action area (replaces default create button)
...
export function GenericDataTable<TData, TFilters extends BaseFilters>({
  ...
  const table = useReactTable({
    ...
    state: {
      ...
      rowSelection,
      pagination: {
        pageIndex: (filters.page || 1) - 1, // Convert to 0-based
        ...
        <div className="flex h-full w-full flex-col min-w-0">
          ...
          <div className="flex-1 min-h-0 min-w-0 overflow-hidden">
            <div className="h-full w-full overflow-auto">
              <Table
                containerClassName={loading || !table.getRowModel().rows?.length ? "h-full w-full" : "h-0 min-h-full w-full"}
